# VYNEX — LinkedIn Outreach Playbook

## Target ICP
- Agenti di commercio monomandatari / plurimandatari
- 35-55 anni
- Nord Italia (maggiore concentrazione: Lombardia, Veneto, Emilia-Romagna, Piemonte)
- Settori: ferramenta/utensileria, prodotti industriali, alimentari, medicali, abbigliamento, arredo
- Caratteristica: attivi su LinkedIn, spesso lamentano burocrazia e admin

## Ricerca LinkedIn
Query di ricerca:
- "agente di commercio" + città
- "rappresentante di commercio"
- "sales agent" + "Italy"
- Gruppi: "Agenti di Commercio Italia", "FNAARC", "Confcommercio agenti"

---

## Messaggio 1 — Cold (connessione + messaggio)

**Oggetto connessione:** (nessun messaggio, solo connessione)

**Dopo accettazione (giorno +1):**

```
Ciao [Nome],

ho visto che sei agente di commercio — lavori nel settore [settore dal profilo]?

Sto sviluppando VYNEX, uno strumento che ho costruito specificamente per agenti come te: descrivi la visita con voce o testo (2 minuti), e in 30 secondi ottieni report di visita, email di follow-up e offerta commerciale pronti da inviare.

Niente più serate a scrivere report.

È gratis fino a 10 documenti al mese. Posso mandarti il link?

[Nome]
```

---

## Messaggio 2 — Follow-up (giorno +4 se no risposta)

```
Ciao [Nome],

senza pressione — so che gli agenti non hanno tempo da perdere.

In 30 secondi: provi VYNEX gratis qui → https://agentia-production-fb78.up.railway.app

Se non fa per te, nessun problema. Se invece ti fa risparmiare anche solo un'ora a settimana, vale la prova.

[Nome]
```

---

## Messaggio 3 — Se risponde con interesse

```
Perfetto [Nome]!

Registrati qui (gratis, no carta): https://agentia-production-fb78.up.railway.app/registrati

Come funziona:
1. Scrivi la visita come la racconteresti a un collega
2. In 30 secondi hai i 3 documenti pronti
3. Copi, modifichi se vuoi, invii

Se hai domande o vuoi che ti faccia vedere dal vivo come funziona, sono qui.

[Nome]
```

---

## Gestione obiezioni comuni

**"Non uso molto LinkedIn / non ho tempo"**
→ "Capisco perfettamente. Ti mando solo il link: vynex.it — 2 minuti per provare, gratis."

**"Uso già un CRM"**
→ "VYNEX non è un CRM — si integra con il tuo flusso di lavoro. È il passo PRIMA di inserire dati nel CRM: genera il report, poi lo copi dove vuoi."

**"Non sono bravo con la tecnologia"**
→ "È semplicissimo: scrivi come scrivi un SMS, l'AI fa il resto. Se sai usare WhatsApp, sai usare VYNEX."

**"Costa troppo"**
→ "Parte gratis, 10 documenti/mese. Se ti fa risparmiare 5 ore al mese, il Pro a €49 lo ripaga in meno di un'ora di lavoro."

**"Non mi fido dei dati"**
→ "I tuoi testi vengono usati solo per generare i documenti, non vengono conservati per training AI o condivisi. Privacy policy completa su /privacy."

---

## Volume e KPI target

**Settimana 1-2:**
- 50 connessioni/giorno (limite LinkedIn: ~100/giorno)
- Target: 500 connessioni totali
- Accept rate atteso: 30-40% → ~175 conversazioni

**Settimana 3-4:**
- Follow-up su chi ha accettato ma non risposto
- Target conversioni free: 5-10% → 8-17 utenti free
- Target conversioni pro: 10-20% degli utenti free → 1-3 paganti

**Primo MRR target:** €49-147 entro fine mese 1
**Target mese 3:** 50 Pro = €2.450 MRR

---

## Canali alternativi

### Facebook Groups
- "Agenti di Commercio - Forum Italia" (15k+ membri)
- Messaggio non commerciale: condividere un post utile + link in bio
- Post tipo: "Ho costruito qualcosa per agenti di commercio — 30 secondi per 3 documenti. Chi vuole testarlo gratis?"

### Forum FNAARC / Confcommercio
- Pubblica articolo utile su risparmio tempo
- Link alla landing page nel profilo

### YouTube SEO
- Video: "Come scrivere un report di visita perfetto (con AI)"
- Target: "agente di commercio report visita", "email follow-up cliente"
- Link in descrizione

### Email diretta (futuro)
- Acquisto database agenti di commercio italiani (EnrichedB2B, Kompass Italia)
- ~200.000 agenti registrati all'albo
